...
async def backfill_dest(client: Client, dest: str, channel_id: int) -> None:
    ...
    logger.info(f"[{dest.upper()}] This may take a while for large channels...")
    ...
    async for msg in client.get_chat_history(channel_id, limit=0):
        ...
        if len(operations) >= 100:
            ...
            operations.clear()
            logger.info(f"  [{dest.upper()}] ...{total_media} media processed so far")
...
async def _flush(vault, operations: list, dest: str) -> None:
    try:
        ...
        if result.upserted_count:
            logger.info(f"  [{dest.upper()}] inserted {result.upserted_count} new, "
                  f"matched {result.matched_count} existing (skipped)")
    except BulkWriteError as e:
        inserted = e.details.get("nInserted", "?")
        logger.info(f"  [{dest.upper()}] partial write — {inserted} inserted (duplicates silently skipped)")
...
async def _resolve_channel(client: Client, raw_id, label: str) -> int | None:
    ...
    try:
        ...
        logger.info(f"❌ [{label}] Invalid channel ID in .env: {raw_id!r}")
        return None

    logger.info(f"  Resolving {label} ({channel_id})...", end=" ", flush=True)
    try:
        chat = await client.get_chat(channel_id)
        logger.info(f"✅  '{chat.title}' (type={chat.type})")
        return chat.id  # use Pyrogram's confirmed ID (now cached in session)
    except Exception as e:
        logger.info(f"\n  ❌ Cannot resolve {label} ({channel_id}): {e}")
        logger.info(f"     Make sure the logged-in account is a MEMBER of this channel.")
        return None


async def main() -> None:
    logger.info("=" * 60)
    logger.info("  VaultFlow Backfill Tool")
    logger.info("=" * 60)
    ...
    logger.info("You will be prompted for your phone number and OTP on first run.\n")
    await client.start()
    ...
    logger.info("Syncing dialogs (this caches all your channels)...")
    async for dialog in client.get_dialogs():
        title = (dialog.chat.title or "").lower()
        if "premium" in title or "vault" in title or "wild" in title:
            logger.info(f"  Found: '{dialog.chat.title}' → id={dialog.chat.id}")
    ...
    logger.info("  Scheduler picks up queued content within 60 seconds.")
    logger.info("  Daily cap in .env controls posting rate.")
    logger.info("  You can delete 'backfill_user_session.session' now.")
    logger.info("=" * 60)
